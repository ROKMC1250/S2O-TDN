...
def save_images(webpage, visuals, image_path, image_original_path, max_value= 2**8, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        util.save_gdal_image(im, save_path, "uint8", image_original_path)
        ims.append(image_name)
        txts.append(label)
        links.append(image_name)
    webpage.add_images(ims, txts, links, width=width)
